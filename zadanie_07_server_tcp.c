...
void* server_main(void* _server_data)
{
    ...
    rv = listen(sfd, 5);
    if (rv != 0) {
        perror("Socket listen for main server.\n");
        close(sfd);
        abort();
    }
    ...
    while (1) {
        ...
        for (i = 0; i < n; i++) {
                    ...
                    (!(events[i].events & EPOLLIN)))
            {
                ...
                while (1) {
                    ...
                    if (infd == -1) {
                        if ((errno == EAGAIN) ||
                            ...
                            perror("Main server accept connection.\n");
                            break;
                        }
                    }
                    ...
                    if (s == 0) {
                        printf("Main server accepted connection on descriptor %d"
                                "(host=%s, port=%s)\n", infd, hbuf, sbuf);
                    }
                    ...
                    if (s == -1){
                        perror("main add connection epoll_ctl\n");
                        close(sfd);
                        close(efd);
                        abort();
                    }
                ...
                if (cfd == -1) {
                    perror("Socket client creation for main server.\n");
                    abort();
                }

                rv = connect(cfd, (struct sockaddr*) &sub_addr, sizeof(sub_addr));
                //TODO: try non-blocking?
                if (rv != 0){
                    perror("Send to sub from main error.\n");
                    close(sfd);
                    close(efd);
                    close(cfd);
                    abort();
                ...
                if(done) {
                    printf("Main server closed connection on descriptor %d\n",
                            events[i].data.fd);
                    close(cfd);
...
void* server_sub(void* _server_data)
{
    ...
    rv = listen(sfd, 5);
    if (rv != 0) {
        perror("Socket listen for sub server.\n");
        close(sfd);
        abort();
    }
    ...
    while (1) {
        ...
        for (i = 0; i < n; i++) {
                    ...
                    (!(events[i].events & EPOLLIN)))
            {
                ...
                while (1) {
                    ...
                    if (infd == -1) {
                        if ((errno == EAGAIN) ||
                            ...
                            perror("Sub server accept connection.\n");
                            break;
                        }
                    }
                    s = getnameinfo(&in_addr, in_len,
                                    ...
                                    NI_NUMERICHOST | NI_NUMERICSERV);
                    if (s == 0) {
                        printf("Sub server accepted connection on descriptor %d"
                                "(host=%s, port=%s)\n", infd, hbuf, sbuf);
                    }
                ...
                if(done) {
                    printf("Sub closed connection on descriptor %d\n", events[i].data.fd);
                    close(events[i].data.fd);
                }
            }
        }
